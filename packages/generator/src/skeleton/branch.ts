import { noise, interpolateArray, interpolateSkeleton } from 'helper';
...
export default function createBranchSkeleton(
  ...
  const branchAmount = parameters.amount || 3;
  const gravity = parameters.gravity || 0;
  const lowestBranch = getLowestBranch(parameters.lowestBranch, i);

  const branchLengthArray = getBranchLengthArray(parameters.length);
  const branchLengthVariation = parameters.length.variation || 0;

  const branchOffset = parameters.offset.value || 0;
  const branchOffsetVariation = parameters.offset.variation || 0;

  const branchAngleArray = getArrayFromParam(parameters.angle);
  const branchAngleVariation = parameters.angle.variation || 0;

  const branchRotation = parameters.rotation.value || 0;
  const branchRotationVariation = parameters.rotation.variation || 0;

  const branchNoiseStrengthArray = getArrayFromParam(parameters.noiseStrength);
  const branchNoiseStrength = parameters.noiseStrength.value;
  const branchNoiseScale = parameters.noiseScale || 1;

  const branchDistance = (1 - lowestBranch) / branchAmount;

  const skeletonLength = skeletons.length / 3;
  ...
  for (let j = 0; j < branchAmount; j++) {
    ...
    const origin = new Vec3().fromArray(
      interpolateSkeleton(skeletons, positionAlongStem),
    );
    ...
    skeletons[j] = new Float32Array(amountPoints * 3);
    ...
    for (let k = 0; k < amountPoints; k++) {
      ...
      skeletons[j][k * 3 + 0] = origin[0] + __x;
      skeletons[j][k * 3 + 1] = origin[1] + __y;
      skeletons[j][k * 3 + 2] = origin[2] + __z;
    }
