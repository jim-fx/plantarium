...
export default class ProjectManager extends EventEmitter<EventMap> {
  ...
  private async saveProject(_p: Project) {
    ...
    project.createdAt =
      typeof project.createdAt === 'string'
        ? project.createdAt
        : (project.createdAt.toISOString() as unknown as Date);
    project.updatedAt =
      typeof project.updatedAt === 'string'
        ? project.updatedAt
        : (project.updatedAt.toISOString() as unknown as Date);

    const errors = validator.isPlantProject(project);
    if (errors?.length) {
      createToast(errors[0], {
        title: "Can't save project " + project?.meta?.name || project.id,
        type: 'warning'
      });
  ...
  async getProject(id: string): Promise<Project> {
    ...
    const project = await storage.getItem<Project>(PTP_PREFIX + id);
    if (!project) {
      console.trace("missing project: " + id)
    }
  ...
  async renderThumbnail(
    projectId: string,
    { project, geo }: { project?: Project; geo?: TransferGeometry }
  ) {
  ...
  private async loadProjects() {
    ...
    if (!projectIds.length && browser) {
      ...
      await this.saveProject(project);
      createToast('Because you had no projects, (yet) I loaded the Fern Simple example for you', {
        type: 'success'
      });
    }
