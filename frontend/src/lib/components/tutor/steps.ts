...
export async function settings(s: string) {
	...
	let res = await createOverlay({
		description: 'This is the <b>SettingView</b>, here you can change some general settings.',
		selector: 'header > div.right > div > .hover-wrapper > .wrapper'
	});
...
export async function levelNodeIntro(s: string) {
	...
	const res = await createOverlay({
		...
		description: `This is the <b>Node View</b>, here you define the way your plant looks. <br> If you are not so sure how nodes work, maybe visit the <b>${visit}</b>.`,
...
export async function plantNodes(s: string) {
	...
	if (level === 0) {
		await createAlert(
			`This is the introduction to the <b>Plantarium</b> NodeSystem. <br>. You can also get here by using the <b>?</b> panel on the top right.`,
			{ values: ['Lets Go'] }
		);

		await createToast(
			`First, lets make sure that we start with a clean project, be opening the project manager <b>QuickSelect</b> menu and clicking <b>new</b>`,
			{ values: ['Done!'] }
		);
	...
	if (level === 1) {
		let res = await createToast(
			`You should see a single stem on the left side, and two nodes on the right side. <br><br> Let's try and create a simple plant. We start by rotating the stem on the right side, press <b>[shift+a]</b>, search for <i>rotate</i> and press <b>[enter]</b>`,
			{ values: ['Done'] }
		);
		if (!res) return 'final';

		res = await createToast(
			`Click on the new node and drag it onto the connection between <i>stem</i> and <i>output</i>. If the nodes are to close together you can click on their name and move them.`,
			{ values: ['Done'] }
		);
		if (!res) return 'final';

		await createToast(
			`<i>Tip:</i> Press <b>?</b> on the keyboard and hover over nodes to find out what they do`,
			{ timeout: 5000 }
		);

		res = await createToast(
			`Set the axis to <b>x</b>, disable <b>spread</b> and increase the angle to around 0.6`,
			{ values: ['Done'] }
		);
		if (!res) return 'final';

		res = await createToast(
			`Now we increase the <b>amount</b> on the stem node to something like 5. <br>We dont see any change because all the stems are in exactly the same place.`,
			{ values: ['Done'] }
		);
		if (!res) return 'final';

		res = await createToast(
			`Create a new <b>Rotate</b> node and connect it between the other rotate node and the output. Set the axis to y and increase the angle. <br> You should now see all the other stems.`,
			{ values: ['Done'] }
		);
		if (!res) return 'final';
		...
		res = await createToast(
			`Now, create a <b>gravity</b> node and connect it between the last rotate node and the output, set the strength to ~0.5`,
			{ values: ['Done'] }
		);
		if (!res) return 'final';

		res = await createToast(
			`For the final touch create an <b>instance</b> node and connect it between the gravity and the output`,
			{ values: ['Done'] }
		);
		if (!res) return 'final';

		res = await createToast(
			'The node will complain that it needs a model to instance, create a <b>leaf</b> node and connect it to the model input of the instance node',
			{ values: ['Done'] }
		);

		await createToast(
			`<i>Tip:</i> when you drag a node socket while holding <b>[ctrl]</b> and release, it shows you a menu of available nodes`,
			{ values: ['Cool!'] }
		);
