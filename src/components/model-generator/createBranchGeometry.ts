...
function getBranchDiameter(diameter: parameter, stemDiameter: parameter, i: number) {
  let v = 0.1;

  if ("variation" in stemDiameter) {
    v = diameter.value - diameter.value * stemDiameter.variation * ((noise.n1d(93815 + i * 200) + 1) / 2);
  } else {
    v = diameter.value;
  }

  if (diameter.curve && diameter.curve.length) {
    return curveToArray(diameter.curve).map((_v: number) => v * _v * stemDiameter.value);
  } else {
...
export default function(pd: plantDescription, settings: settings, skeletons: Float32Array[], i: number): TransferGeometry {
  ...
  return join(
    ...skeletons.map((skeleton, i) => {
      return tube(skeleton, diameter.map(v => v * ((1 - i / amount) * (1 - lowestBranch))), resX);
    })
  );
}
