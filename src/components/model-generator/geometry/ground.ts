...
export default function(size: number, resX: number = 12, resY: number = 12): TransferGeometry {
  //General parameters
  ...
  const xPositions = new Array(resX - 1).fill(null).map((v, i, a) => size + (i / (a.length - 1)) * slopeLength);
  xPositions.push(totalSize);

  const yPositions = xPositions.map((v, i, a) => {
    const slopeAlpha = (Math.max(Math.min(v, size + slopeLength), size) - size) / slopeLength;
